...
import { createMatchers, Matcher } from "./matcher.ts";

import { FontStyle, ThemeTrieElementRule } from "./theme.ts";
import { IOnigLib, IRawGrammar, IRawRepository, IRawRule } from "./types.ts";
import {
  IEmbeddedLanguagesMap,
  IGrammar,
  IStackElement as StackElementDef,
  IToken,
  ITokenizeLineResult,
...
const UseOnigurumaFindOptions = false;
...
export enum TemporaryStandardTokenType {
  Other = 0,
  Comment = 1,
  String = 2,
  RegEx = 3,
...
function _extractIncludedScopesInPatterns(
  ...
  for (const pattern of patterns) {
    ...
    const patternRepository = pattern.repository
      ? { ...repository, ...pattern.repository }
      : repository;

    if (Array.isArray(pattern.patterns)) {
      _extractIncludedScopesInPatterns(
        result,
        baseGrammar,
        selfGrammar,
        pattern.patterns,
    ...
    if (include === "$base" || include === baseGrammar.scopeName) {
      ...
      if (sharpIndex >= 0) {
        ...
        if (scopeName === baseGrammar.scopeName) {
          ...
          result.add(
            new PartialScopeDependency(
              scopeName,
              include.substring(sharpIndex + 1),
            ),
          );
        }
      } else {
...
function collectDependenciesForDep(
 ...
 */
function collectSpecificDependencies(
  result: ScopeDependencyCollector,
  baseGrammar: IRawGrammar,
  selfGrammar: IRawGrammar,
  include: string,
  repository: IRawRepository | undefined = selfGrammar.repository,
): void {
...
export class ScopeMetadata {
  ...
  constructor(
    ...
    themeData: ThemeTrieElementRule[] | null,
  ) {
...
class ScopeMetadataProvider {
  ...
  constructor(
    ...
    if (embeddedLanguages) {
      ...
      for (let i = 0, len = scopes.length; i < len; i++) {
        ...
        if (typeof language !== "number" || language === 0) {
          console.warn(
            "Invalid embedded language found at scope " + scope + ": <<" +
              language + ">>",
          );
    ...
    // create the regex
    const escapedScopes = Object.keys(this._embeddedLanguages).map((
      scopeName,
...
export class Grammar implements IGrammar, IRuleFactoryHelper, IOnigLib {
  ...
  private _collectInjections(): Injection[] {
    ...
    dependencyProcessor.seenFullScopeRequests.forEach((scopeName) => {
      ...
      if (this._grammarRepository) {
        const injectionScopeNames = this._grammarRepository.injections(
          scopeName,
        );
        if (injectionScopeNames) {
          injectionScopeNames.forEach((injectionScopeName) => {
            const injectionGrammar = this.getExternalGrammar(
              injectionScopeName,
            );
            if (injectionGrammar) {
  ...
  private _tokenize(
    ...
    if (!prevState || prevState === StackElement.NULL) {
      isFirstLine = true;
      const rawDefaultMetadata = this._scopeMetadataProvider
        .getDefaultMetadata();
      ...
      const rawRootMetadata = this._scopeMetadataProvider.getMetadataForScope(
        rootScopeName,
      );
      ...
      const scopeList = new ScopeListElement(
        null,
        rootScopeName === null ? "unknown" : rootScopeName,
        rootMetadata,
      );
...
function handleCaptures(
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (captureRule.retokenizeCapturedWithRuleId) {
      ...
      const nameScopesList = stack.contentNameScopesList.push(
        grammar,
        scopeName,
      );
    ...
    if (captureRuleScopeName !== null) {
      // push
      const base = localStack.length > 0
        ? localStack[localStack.length - 1].scopes
...
function matchInjections(
  ...
  for (let i = 0, len = injections.length; i < len; i++) {
    ...
    const matchResult = ruleScanner.scanner.findNextMatchSync(
      lineText,
      linePos,
      findOptions,
    );
...
function matchRule(
  ...
  const r = ruleScanner.scanner.findNextMatchSync(
    lineText,
    linePos,
    findOptions,
  );
  ...
  if (DebugFlags.InDebugMode) {
    ...
    if (r) {
      console.log(
        `matched rule id: ${ruleScanner.rules[r.index]} from ${
          r.captureIndices[0].start
        } to ${r.captureIndices[0].end}`,
      );
    }
  }
...
function _checkWhileConditions(
  ...
  for (
    ...
    if (r) {
      ...
      if (DebugFlags.InDebugMode) {
        console.log(
          "  popping " + whileRule.rule.debugName + " - " +
...
function _tokenizeString(
  ...
  function scanNext(): void {
    if (DebugFlags.InDebugMode) {
      ...
      console.log(
        `@@scanNext ${linePos}: |${
          lineText.content.substr(linePos).replace(/\n$/, "\\n")
        }|`,
      );
    }
    ...
    if (matchedRuleId === -1) {
      ...
      if (_rule instanceof BeginEndRule) {
        ...
        if (DebugFlags.InDebugMode) {
          console.log(
            "  matched " + matchingRule.debugName + " - " +
...
export class ScopeListElement {
  ...
  private static _equals(
    a: ScopeListElement | null,
    b: ScopeListElement | null,
  ): boolean {
    do {
      if (a === b) {
        return true;
  ...
  private static _matchesScope(
    scope: string,
    selector: string,
    selectorWithDot: string,
  ): boolean {
    return selector === scope ||
      scope.substring(0, selectorWithDot.length) === selectorWithDot;
  }

  private static _matches(
    target: ScopeListElement | null,
  ...
  public static mergeMetadata(
    metadata: number,
    scopesList: ScopeListElement | null,
  ...
  private static _push(
    ...
    for (let i = 0, len = scopes.length; i < len; i++) {
      ...
      const metadata = ScopeListElement.mergeMetadata(
        target.metadata,
        target,
        rawMetadata,
      );
...
export class StackElement implements StackElementDef {
  ...
  public static NULL = new StackElement(
    null,
    0,
    0,
    0,
  ...
  private static _structuralEquals(
    ...
    do {
      ...
      if (
        a.depth !== b.depth || a.ruleId !== b.ruleId || a.endRule !== b.endRule
      ) {
        return false;
      }
  ...
  public setContentNameScopesList(
    contentNameScopesList: ScopeListElement,
  ): StackElement {
  ...
  public produceFromScopes(
    ...
    if (this._emitBinaryTokens) {
        ...
        for (const tokenType of this._tokenTypeOverrides) {
          if (tokenType.matcher(scopes)) {
            metadata = StackElementMetadata.set(
              metadata,
              0,
              toTemporaryType(tokenType.type),
              FontStyle.NotSet,
              0,
              0,
            );
      ...
      if (DebugFlags.InDebugMode) {
        ...
        console.log(
          "  token: |" +
            this._lineText!.substring(this._lastTokenEndIndex, endIndex)
              .replace(/\n$/, "\\n") +
            "|",
        );

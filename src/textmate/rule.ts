...
const CAPTURING_REGEX_SOURCE = /\$(\d+)|\${(\d+):\/(downcase|upcase)}/g;
const HAS_BACK_REFERENCES = /\\(\d+)/;
const BACK_REFERENCING_END = /\\(\d+)/g;

function hasCaptures(regexSource: string | null): boolean {
  if (regexSource === null) {
    return false;
  }
...
export abstract class Rule {
  ...
  constructor(
    ...
    this._nameIsCapturing = hasCaptures(this._name);
    this._contentName = contentName || null;
    this._contentNameIsCapturing = hasCaptures(this._contentName);
  }
  ...
  public getName(
    ...
    return replaceCaptures(this._name, lineText, captureIndices);
  }
  ...
  public getContentName(
    ...
    return replaceCaptures(this._contentName, lineText, captureIndices);
  }

  public abstract collectPatternsRecursive(
...
export class CaptureRule extends Rule {
  ...
  public collectPatternsRecursive(
    grammar: IRuleRegistry,
    out: RegExpSourceList,
    isFirst: boolean,
  ) {
...
export class BeginWhileRule extends Rule {
  ...
  public compileWhile(
    ...
    return this._getCachedCompiledWhilePatterns(grammar, endRegexSource)
      .compile(grammar);
  }
    ...
    if (!desc.id) {
      helper.registerRule((id) => {
        ...
        if (typeof desc.begin === "undefined") {
          if (desc.repository) {
            repository = { ...repository, ...desc.repository };
          }
      ...
      for (let i = 0, len = patterns.length; i < len; i++) {
        ...
        if (pattern.include) {
          if (pattern.include.charAt(0) === "#") {
            ...
            pattern.include === "$base" || pattern.include === "$self"
          ) {
            ...
            if (sharpIndex >= 0) {
              ...
              externalGrammarInclude = pattern.include.substring(
                sharpIndex + 1,
              );
            } else {

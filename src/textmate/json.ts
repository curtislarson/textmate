...
function doFail(streamState: JSONStreamState, msg: string): void {
  ...
  throw new Error(
    "Near offset " + streamState.pos + ": " + msg + " ~~~" +
...
export function parse(
  ...
  while (nextJSONToken(streamState, token)) {
    ...
    if (
      state === JSONState.DICT_STATE || state === JSONState.DICT_STATE_NO_CLOSE
    ) {
      if (
        state === JSONState.DICT_STATE &&
    ...
    if (
      state === JSONState.ARR_STATE || state === JSONState.ARR_STATE_NO_CLOSE
    ) {
      if (
        state === JSONState.ARR_STATE &&
...
function nextJSONToken(_state: JSONStreamState, _out: JSONToken): boolean {
  ...
  do {
    ...
    if (
      chCode === ChCode.SPACE || chCode === ChCode.HORIZONTAL_TAB ||
  ...
  if (chCode === ChCode.QUOTATION_MARK) {
    ...
    _out.value = source.substring(_out.offset + 1, pos - 1).replace(
      /\\u([0-9A-Fa-f]{4})/g,
      (_, m0) => {
